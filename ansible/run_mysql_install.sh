...
if [ "$VAR_MYSQL_VERSION" -gt 0 -a "$VAR_HOST" != "" ]; then
  ...
  ### mysql install ####
  ansible-playbook -v -i $SCRIPT_PATH/hosts -e "{mysql_version: '$VAR_MYSQL_VERSION', mysql_minor_version: '$VAR_MYSQL_MINOR_VERSION'}" $SCRIPT_PATH/playbook/mysql_install.yml -l $VAR_HOST
else
