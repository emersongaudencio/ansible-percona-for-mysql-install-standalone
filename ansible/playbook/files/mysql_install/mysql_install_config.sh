...
mysqld --defaults-file=/etc/my.cnf $EXTRA --user=mysql
sleep 5
...
RD_MYSQLCHK_USER_PWD="$CLIENT_PREFFIX-mysqlchk-$SERVERID"
...
mysql -e "CREATE USER '$MYSQLCHK_USER_NAME'@'%' IDENTIFIED BY '$MYSQLCHK_USER_PWD'; GRANT PROCESS ON *.* TO '$MYSQLCHK_USER_NAME'@'localhost';";
